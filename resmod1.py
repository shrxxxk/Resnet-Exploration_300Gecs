...
# --- NEW PRE-ACTIVATION RESNET MODEL DEFINITION ---

# <-- ADDED: Pre-activation Basic Block -->
class PreActBasicBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, downsample=None):
        super(PreActBasicBlock, self).__init__()
        # Pre-activation: BN and ReLU come BEFORE the convolution
    ...
    def forward(self, x):
        ...
        out += identity
        # Note: The final ReLU is missing. It will be the first operation of the next block.
        return out

# <-- ADDED: Pre-activation ResNet-34 Architecture -->
class PreActResNet34(nn.Module):
    def __init__(self, num_classes=5):
        super(PreActResNet34, self).__init__()
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
        # The first BN and ReLU are moved into the stages
        ...
        # Final BN and ReLU after all blocks
    ...
    def _make_stage(self, in_channels, out_channels, num_blocks, stride):
        ...
        layers = [PreActBasicBlock(in_channels, out_channels, stride, downsample)]
        for _ in range(1, num_blocks):
            layers.append(PreActBasicBlock(out_channels, out_channels))
...
# <-- ADDED: Factory function for the new model -->
def create_preact_resnet34(num_classes=5):
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("INITIALIZING PRE-ACTIVATION RESNET-34 MODEL FOR TRAINING") # <-- MODIFIED print statement
    print("="*60)
    model = create_preact_resnet34(num_classes=num_classes).to(device) # <-- MODIFIED: Using the new model
    ...
    best_val_acc = 0.0
    model_save_path = 'preact_resnet34_best_model.pth' # <-- MODIFIED model name
    ...
    best_model = create_preact_resnet34(num_classes=num_classes).to(device) # <-- MODIFIED: Use new model
