...
# --- TAMBAHKAN DEFINISI BLOK INI BERSAMA DEFINISI MODEL LAINNYA ---

# --- DEFINISI BLOK BARU: KOMBINASI PRE-ACTIVATION DAN MULTI-PATH ---

class MultiPathPreActBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, downsample=None):
        super(MultiPathPreActBlock, self).__init__()
        
        # Pre-activation tetap sama
        self.bn1 = nn.BatchNorm2d(in_channels)
        
        # --- MODIFIKASI: Conv1 dipecah menjadi jalur paralel ---
        # Setiap jalur akan menghasilkan setengah dari channel output yang diinginkan
        half_out_channels = out_channels // 2
        
        # Jalur 1 (3x3)
        self.conv1_3x3 = nn.Conv2d(in_channels, half_out_channels, kernel_size=3, stride=stride, padding=1, bias=False)
        
        # Jalur 2 (5x5)
        self.conv1_5x5 = nn.Conv2d(in_channels, half_out_channels, kernel_size=5, stride=stride, padding=2, bias=False)
        
        # Konvolusi 1x1 untuk menyatukan kembali channel setelah digabung
        # Inputnya adalah half_out_channels * 2 = out_channels
        self.conv1_merge = nn.Conv2d(out_channels, out_channels, kernel_size=1, stride=1, bias=False)
        # --------------------------------------------------------

        # Sisa dari blok tetap sama seperti PreActBasicBlock
    ...
    def forward(self, x):
        ...
        # --- MODIFIKASI: Lewatkan ke jalur paralel ---
        path1 = self.conv1_3x3(out)
        path2 = self.conv1_5x5(out)
        
        # Gabungkan hasil dari kedua jalur
        out = torch.cat([path1, path2], dim=1)
        
        # Satukan channel dengan conv 1x1
        out = self.conv1_merge(out)
        # -----------------------------------------------
        ...
        out += identity
        # ReLU final akan dilakukan oleh blok berikutnya (ciri khas pre-activation)
        return out

# --- DEFINISI MODEL BARU ---

class PreActMultiPathResNet(nn.Module):
    def __init__(self, num_classes=5):
        super(PreActMultiPathResNet, self).__init__()
        # Bagian awal model sama persis
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        
        # Stage-stage ini sekarang menggunakan blok baru kita
        ...
        # Bagian akhir model juga sama persis
    ...
    def _make_stage(self, in_channels, out_channels, num_blocks, stride):
        ...
        # --- PERBEDAAN UTAMA: Panggil MultiPathPreActBlock ---
        layers = [MultiPathPreActBlock(in_channels, out_channels, stride, downsample)]
        for _ in range(1, num_blocks):
            layers.append(MultiPathPreActBlock(out_channels, out_channels))
        # ----------------------------------------------------
            
        return nn.Sequential(*layers)
    
    # Fungsi _initialize_weights dan forward tetap sama persis dengan PreActResNet34
...
# Buat juga factory function-nya
def create_preact_multipath_resnet34(num_classes=5):
    return PreActMultiPathResNet(num_classes=num_classes)
...
if __name__ == "__main__":
    ...
    # 1. Persiapan Awal (Device, Hyperparameters, Data)
    ...
    # 2. Inisialisasi Model Gabungan (PreAct + MultiPath)
    print("\n" + "="*60)
    print("INITIALIZING PRE-ACTIVATION MULTI-PATH RESNET FOR TRAINING")
    print("="*60)
    
    model = create_preact_multipath_resnet34(num_classes=num_classes).to(device)
    model_save_path = 'preact_multipath_resnet34_best_model.pth'
    
    summary(model, input_size=(batch_size, 3, img_size[0], img_size[1]))
    
    # 3. Persiapan Proses Pelatihan
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=weight_decay)
    
    history = {'train_loss': [], 'train_acc': [], 'val_loss': [], 'val_acc': []}
    patience = 5  # Berhenti jika val_acc tidak membaik selama 5 epoch
    epochs_no_improve = 0
    best_val_acc = 0.0
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            epochs_no_improve += 1 # Tambah counter jika tidak ada peningkatan
    ...
    best_model = create_preact_multipath_resnet34(num_classes=num_classes).to(device)
