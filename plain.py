...
# All classes and functions that might be needed by any process go here.
...
class MakananIndo(Dataset):
    ...
    def __init__(self, data_dir, img_size, transform=None, split='train'):
        ...
        # Shuffle indices once for a consistent train/val split
...
class PlainBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            identity = self.downsample(x) # Corrected: downsample input x, not identity
        # No residual connection
        out = F.relu(out)
        return out

# Model Definition: Plain34
...
if __name__ == "__main__":
    ...
    # --- Data Loading ---
    print("Loading datasets... (This will run only ONCE)")
    ...
    print("\nCreating label encoder... (This will run only ONCE)")
    ...
    print("\n" + "="*60)
    print("INITIALIZING PLAIN-34 MODEL FOR TRAINING")
    print("="*60)
    ...
    best_val_acc = 0.0
    model_save_path = 'plain34_best_model.pth'
    ...
    best_model = create_plain34(num_classes=num_classes).to(device)
